...
workflow call_cnvs {
    ...
    call plot_segments {
        input:
            sample_id = sample_id,
            chrom_sizes = chrom_sizes,
            read_cov_bin = call_segments.read_cov_bin,
            segments = call_segments.segments
    }
    ...
    output {
        ...
        File segments_plot = plot_segments.segments_plot
    }
}
...
task calc_bin_coverage {
    ...
    Int disk_space = ceil(
        1.5 * size(cram_bam, "GiB") + size(ref_fasta, "GiB") + size(wgs_bins_bed, "GiB")
...
task plot_segments {
    meta {
        descriptions: "Generate a GATK-style segments plot"
        allowNestedInputs: true
    }
    ...
    parameter_meta {
        ...
        # outputs
        segments_plot: "PNG plot of segments"
    }
    ...
    command <<<
        ...
        Rscript /app/plot_segments.R \
            "~{chrom_sizes}" \
            "~{read_cov_bin}" \
            "~{segments}" \
            "~{sample_id}.cn_segments.png" \
    >>>

    output {
        File segments_plot = "~{sample_id}.cn_segments.png"
    }
