...
workflow align_wgs_sample {
    meta {
        description: "Produce an analysis-ready, optionally BQSR-recalibrated BAM from CRAM or BAM input"
    ...
    parameter_meta {
        ...
        input_type: "type of input data: CRAM or BAM"
        input_cram_bam: "input CRAM or BAM file"
    ...
    Array[File]+ aligned_bams = select_first([rg_aligned_bams, []])  # !NonemptyCoercion
    ...
    if (run_bqsr) {
        ...
        if (!select_first([check_bqsr_and_oq_tags.bqsr_performed, false]) || (rerun_bqsr && select_first([check_bqsr_and_oq_tags.has_oq_tags, false]))) {
...
task split_bam_by_read_group {
    ...
    Int disk_gb = ceil(size(input_cram_bam, "GiB") * 20) + 50 + additional_disk_gb
...
task revert_and_validate_sam {
    ...
    Int disk_gb = ceil(size(input_bam, "GiB") * 10 + size(ref_fasta, "GiB")) + 50 + additional_disk_gb
...
task biobambam_bamtofastq {
    ...
    parameter_meta {
        ...
        bam: "input unmapped BAM to convert to FASTQ"
    ...
    input {
        ...
        String docker_image = "us-central1-docker.pkg.dev/depmap-omics/terra-images/biobambam2_seqkit"
        String docker_image_hash_or_tag = ":production"
        Int cpu = 2
        Int mem_gb = 8
        Int preemptible = 1
    ...
    Int disk_gb = ceil(size(bam, "GiB") * 10) + 50 + additional_disk_gb
    ...
    command <<<
        ...
        # seqkit split2 reads R1/R2 in lockstep and writes mate-synced parts, so
        # paired shards cannot drift out of sync the way an independent per-file
        # line split can. It also errors if R1/R2 read counts differ. Output
        # names are split/<rg>_1.part_NNN.fq.gz and split/<rg>_2.part_NNN.fq.gz.
        for r1 in *_1.fq.gz; do
            [ -f "$r1" ] || continue
            rgid="${r1%_1.fq.gz}"
            r2="${rgid}_2.fq.gz"

            echo "Splitting $rgid into $n_shards parts with seqkit"
            seqkit split2 \
                --by-part "$n_shards" \
                --read1 "$r1" \
                --read2 "$r2" \
        ...
        ls -l ./*.fq.gz split/*.fq.gz 2>/dev/null || true
    >>>

    output {
        Array[File] output_fastq1 = glob("split/*_1.part_*.fq.gz")
        Array[File] output_fastq2 = glob("split/*_2.part_*.fq.gz")
...
task bwa {
    meta {
        description: "Align a FASTQ shard with bwa mem and write an unsorted BAM"
        allowNestedInputs: true
    }

    parameter_meta {
        # inputs
        fastq1: "FASTQ to align (forward/R1 reads for paired-end, or the single-end reads)"
        fastq2: "reverse (R2) FASTQ for paired-end input, in matching order with fastq1; omit for single-end"
        ...
        mem_gb_per_fastq_gb: "extra memory (GiB) to request per GiB of input FASTQ, on top of the fixed base/-K/thread terms"
    ...
    input {
        ...
        Float mem_gb_per_fastq_gb = 1.5
    }

    # select_all drops fastq2 when it is absent (single-end), so the size terms below
    # count one or two FASTQs as appropriate.
    Array[File] fastqs = select_all([fastq1, fastq2])
    ...
    Int disk_gb = ceil((size(fastqs, "GiB") * 10) + ref_size) + 50 + additional_disk_gb

    # -K bounds per-batch memory, but peak RSS still grows with total input size in
    # practice (writer-thread queuing, repeat-heavy regions producing more secondary/
    # supplementary records) — bwa mem got OOM-killed mid-write on large unsplit
    # shards even though -K/cpu terms alone said they'd fit, corrupting the piped
    # samtools view output. Scale explicitly with FASTQ size as a safety margin.
    Int mem_gb = 8 +
        ceil(bwa_batch_size / 25000000.0) * 2 +
        ceil(cpu * 0.1) +
        ceil(size(fastqs, "GiB") * mem_gb_per_fastq_gb) +
        additional_memory_gb

    # Paired shards are named <rg>_1.part_NNN.fq.gz by seqkit; drop the mate
    # marker so the output BAM is <rg>.part_NNN.bam. A no-op for single-end
    # shards, whose suffixes are _o1/_o2/_s.
    String out_basename = sub(basename(fastq1, ".fq.gz"), "_1[.]part_", ".part_")

    command <<<
        set -uo pipefail

        # pipefail alone reports only the rightmost non-zero exit code, which hides
        # e.g. bwa getting OOM-killed (137) behind samtools's generic parse-error 1.
        # PIPESTATUS is bash-only (absent when the script runs under POSIX-mode sh),
        # so stash bwa's real exit code to a file from inside the pipeline instead.
        bwa_rc_file=$(mktemp)
        ...
        { bwa mem -K ~{bwa_batch_size} \
            ...
            "~{ref_fasta}" \
            "~{fastq1}" \
            ~{if defined(fastq2) then '"' + select_first([fastq2]) + '"' else ""}; \
          echo $? > "$bwa_rc_file"; } \
        | samtools view \
            -@ ~{cpu} \
            -Shb \
            ...
            -
        samtools_rc=$?
        bwa_rc=$(cat "$bwa_rc_file")

        echo "bwa mem exit code: $bwa_rc" >&2
        echo "samtools view exit code: $samtools_rc" >&2

        if [ "$bwa_rc" -ne 0 ]; then
            exit "$bwa_rc"
        elif [ "$samtools_rc" -ne 0 ]; then
            exit "$samtools_rc"
        fi
    >>>
...
task sort_and_index_markdup_bam {
    ...
    Int computed_mem_mb = ceil(size(input_bam, "MiB") * 0.5) + 12000 + additional_memory_mb
    Int mem_mb = if computed_mem_mb < 48000 then computed_mem_mb else 48000
    Int disk_gb = ceil(size(input_bam, "GiB") * 4) + 10 + additional_disk_gb
