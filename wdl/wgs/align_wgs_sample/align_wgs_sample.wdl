...
task split_bam_by_read_group {
    ...
    command <<<
        ...
        source <(curl -fsSL https://raw.githubusercontent.com/broadinstitute/magicwand/refs/heads/main/install.sh)
        magicwand init
...
task biobambam_bamtofastq {
    ...
    Int disk_gb = ceil(size(bam, "GiB") * 5) + 20 + additional_disk_gb
...
task bwa {
    ...
    # Disk holds the localized FASTQ shard(s), the output BAM (~the same order as the
    # FASTQs), and the resident reference index. Shards are bounded by gb_per_bwa_bam
    # (~4 GiB unmapped BAM, ~2 GiB per FASTQ), so 6x the FASTQ size covers input +
    # output comfortably.
    Int disk_gb = ceil((size(fastqs, "GiB") * 6) + ref_size) + 20 + additional_disk_gb

    # Peak RSS ~= the BWA index resident in RAM (~6 GiB for hg38) + the per-batch
    # working set (the -K batch of reads plus their alignments, growing with thread
    # count) + samtools view compression buffers + OS headroom. At a fixed -K and cpu
    # this is roughly constant regardless of total FASTQ size, so the per-shard term
    # stays small. Shards here are bounded by gb_per_bwa_bam (~2 GiB per FASTQ), giving
    # ~16 GiB. Floor 14000 MiB, cap 32768 MiB (32 GiB) for the rare oversized shard.
    Int computed_mem_mb = 8000 + (cpu * 384) + ceil(size(fastqs, "MiB") * 0.5) + additional_memory_mb
    Int mem_mb = if computed_mem_mb < 14000 then 14000 else if computed_mem_mb > 32768 then 32768 else computed_mem_mb

    # Strips the trailing _1 from paired forward FASTQs; a no-op for single-end
    # shards (whose suffixes are _o1/_o2/_s, which do not end in _1).
    ...
    command <<<
        ...
        # Drop -e around the pipe so we can inspect each stage's exit status:
        # otherwise a samtools parse error masks whether bwa itself failed.
        # fastq2 expands to nothing when absent, making this a single-end alignment.
        set +e
        ...
        if [ "${status[0]}" -ne 0 ]; then
            echo "ERROR: bwa mem failed with exit ${status[0]}:" 1>&2
            cat bwa.stderr 1>&2
            exit "${status[0]}"
        fi
        if [ "${status[1]}" -ne 0 ]; then
            echo "ERROR: samtools view failed with exit ${status[1]} (bwa exited 0, so its SAM output is malformed/truncated):" 1>&2
            cat bwa.stderr 1>&2
            exit "${status[1]}"
        fi
    >>>
