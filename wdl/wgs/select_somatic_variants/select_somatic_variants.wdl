...
workflow select_somatic_variants {
    ...
    parameter_meta {
        ...
        save_sig_variants: "if true, save the `mut_sig_variants` variants data as Parquet"
        save_enriched_variants: "if true, save the `mut_enriched_variants` data as Parquet"
    ...
    input {
        ...
        Boolean save_sig_variants = false
        Boolean save_enriched_variants = true
    }
    ...
    call do_select_somatic_variants {
        input:
            ...
            save_sig_variants = save_sig_variants,
...
task do_select_somatic_variants {
    ...
    command <<<
        ...
        if [ "~{save_sig_variants}" = "true" ]
        then
            sig_line="--mut-sig-out-file=~{sample_id}.mut_sig_variants.parquet"
        else
            sig_line=""
        fi
