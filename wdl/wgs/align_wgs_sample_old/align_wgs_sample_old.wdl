...
#   - `CheckContamination` removed.
#   - `collect_insert_size_metrics` removed.
...
#   - CramToUnmappedBams tasks inlined directly; no subworkflow.
...
workflow align_wgs_sample_old {
    ...
    if (input_type == "BAM" || input_type == "CRAM") {
        if (input_type == "CRAM") {
            Float cram_size = size(select_first([input_cram_bam]), "GiB")

            call CramToBam {
                input:
                    ref_fasta = select_first([delivery_ref_fasta, ref_fasta]),
                    ref_fasta_index = select_first([delivery_ref_fasta_index, ref_fasta_index]),
                    cram_file = select_first([input_cram_bam]),
                    output_basename = basename(select_first([input_cram_bam]), ".cram"),
                    disk_size = ceil(cram_size * 6) + 20
            }
        }

        File cram_to_ubam_input_file = select_first([CramToBam.output_bam, input_cram_bam])
        Float cram_to_ubam_input_size = size(cram_to_ubam_input_file, "GiB")

        if (!defined(output_map)) {
            call GenerateOutputMap {
                input:
                    input_bam = cram_to_ubam_input_file,
                    unmapped_bam_suffix = select_first([unmapped_bam_suffix, ".unmapped.bam"]),
                    disk_size = ceil(cram_to_ubam_input_size) + 20
            }
        }
        ...
        scatter (rg_map_file in SplitUpOutputMapFile.rg_to_ubam_file) {
            call SplitOutUbamByReadGroup {
                input:
                    ...
                    disk_size = ceil(cram_to_ubam_input_size * 2) + 20
            }
            ...
            call RevertSam {
                input:
                    ...
                    disk_size = ceil(cram_to_ubam_input_size * 3) + 20
            }
            ...
            call ValidateSamFile {
                input:
                    ...
                    disk_size = ceil(unmapped_bam_size) + 20
            }
        }

        scatter (ubam in SortSam.output_bam) {
...
task SplitUpOutputMapFile {
    input {
        ...
        String docker_image = "us-central1-docker.pkg.dev/depmap-omics/terra-images/alpine"
        String docker_image_hash_or_tag = ":3.21"
        Int disk_size = 10
        Int memory_in_MiB = 3000
    }
...
task emit_pe_records {
    input {
        ...
        String docker_image = "us-central1-docker.pkg.dev/depmap-omics/terra-images/python"
        String docker_image_hash_or_tag = ":3.8-slim"
        Int preemptible = 3
        Int max_retries = 1
        Int cpu = 1
        Int additional_memory_mb = 0
